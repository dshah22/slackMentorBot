...
function display_help(where, from) {
    slack_functions.say('Hey @' + from.name + '! Here are a list of commands: \'keywords\' and \'available\'. When you\'re ready, use `.mentor {keyword} {issue}` to open a new mentor request. Example: `.mentor javascript How exactly does this callback work?`', where);
}
...
function mentor_request(text, where, from) {
    ...
    if (keyword.toUpperCase() === 'ALL') {
        ...
        if (keyword in teams) {
            ...
            if (assigned === false) {
                slack_functions.say('Sorry, but no one is available to help you for that keyword! But, don\'t worry. We\'ll get someone else right on it.', where);
...
function assign_mentor(assigned_mentor, assigned_to_obj, keyword, issue, where) {
    ...
    if (!assigned_mentor_obj) {
        ...
        slack._apiCall('groups.create', {
            ...
            slack._apiCall('groups.setTopic', {
                channel: group_id,
                topic: issue
            }, function (data) {
...
function process_member_add(text, where, from) {
    if (!is_username_admin(from.name)) {
        slack_functions.say('Whoops, you\'re not an admin! PM @darpan to add yourself as a mentor', where);
        return;
    }
